...
const ProductDetailModal = ({ product, isOpen, onClose }) => {
  ...
  // Parse size attributes dynamically
  const sizeAttr = product.attributes?.find(attr => attr.key.toLowerCase().includes('size'));
  const parsedSizes = sizeAttr 
    ? sizeAttr.value.split(/[,\s/]+/).map(s => s.trim()).filter(Boolean)
    : ['S', 'M', 'L', 'XL', 'XXL'];
  ...
  const getColors = () => {
    ...
    return colorAttr
      ...
      : ['Pista Blue', 'Atelier Black', 'Sage Green', 'Nordic Sand'];
  };
                ...
                <div className="md:col-span-6 space-y-6 pt-4 md:pt-0">
                  ...
                  <div className="space-y-2.5">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-bold tracking-widest text-zinc-400 uppercase">Silhouette Size</span>
                      <button 
                        onClick={() => toast('Size chart: standard European tailoring fits.', { icon: '📏' })}
                  ...
                  {similarProducts.length > 0 && (
                    <div className="space-y-3 pt-6 border-t border-zinc-100">
                      <span className="text-[10px] font-bold tracking-widest text-zinc-400 uppercase block">Similar Silhouettes</span>
