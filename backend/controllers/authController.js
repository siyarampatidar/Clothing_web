...
import { getOtpTemplate, getResetTemplate } from '../templates/emailTemplates.js';
import { validateRegisterInput, validateLoginInput, validateResetPasswordInput } from '../validators/authValidator.js';
...
export const register = async (req, res, next) => {
  try {
    ...
    const emailSent = await sendEmail({
      to: email,
      subject: 'Verify Your Email Address - CJP',
      html: emailHtml,
    });

    res.status(201).json({
      success: true,
      message: 'Registration initiated. A 6-digit verification code was sent to your email.',
      email,
      emailSent: !!emailSent,
    });
  } catch (error) {
...
export const resendOtp = async (req, res, next) => {
  try {
    ...
    const emailSent = await sendEmail({
      to: email,
      subject: 'New Verification Code - CJP',
      html: emailHtml,
    });
  ...
  try {
    ...
    if (!user.isVerified) {
      ...
      return res.status(403).json({
        success: false,
        message: 'Your account is unverified. A verification OTP was sent to your email.',
        unverified: true,
        email: user.email,
      });
    }
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(200).json({
        success: true,
        message: 'If that email address exists in our records, a reset password link has been dispatched.',
      });
    }
    ...
    const emailSent = await sendEmail({
      to: user.email,
      subject: 'Password Reset Request - CJP',
      html: emailHtml,
    });
